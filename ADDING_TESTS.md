...
Основной путь начинается на `/learning`: платформа создаёт starter в
`student_tests/`, открывает его во встроенной IDE, запускает pytest и сохраняет
результат по критериям. Полноценный code-server и локальная IDE остаются нужны
для дебага, Playwright и больших решений.

## Рабочий цикл

1. Откройте `/learning`, выберите урок и прочитайте микро-теорию.
2. Нажмите «Создать тест и открыть IDE» либо создайте файл вручную.
3. Запускайте тест до прохождения baseline, policy и назначенных мутаций.
