...
Это инженерный источник задач и мишеней. Полноценный учебный маршрут находится
на `/learning`: к каждой задаче добавлены микро-теория, стартовый файл, запуск,
критериальная оценка и персональный прогресс. Низкоуровневый каталог остаётся
доступен в UI `/practice` и через `GET /api/practice/catalog`.
