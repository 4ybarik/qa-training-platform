...
Практическая платформа для **автоматизации тестирования** с моделью
**5% микро-теории / 95% исполняемой практики**. Страница `/learning` связывает
каждую тему с реальной мишенью, создаёт стартовый тест прямо в `student_tests/`,
открывает его во встроенной IDE, оценивает baseline и назначенные мутации,
сохраняет критерии попытки и подтверждённый прогресс. Jenkins затем запускает
тот же код из Git и публикует результат в Allure.
...
- Учебный маршрут: <http://localhost:8000/learning>
...
Перед стартом приложения Alembic автоматически обновляет схему БД без удаления
существующих данных. На первом запуске загружается учебный
контент: **50 курсов** по 10 трекам «от простого к сложному» (основы → pytest →
HTTP/API → API полигона → Playwright → UI полигона → данные и интеграции →
контракты и надёжность → безопасность/RBAC → CI/CD), **~100 экзаменов** с
**~500 вопросами** всех типов (одиночный/множественный выбор, текстовый ответ,
drag-and-drop). Описания курсов содержат примеры кода и ссылки на практические
задачи каталога. Обновить контент на существующей базе: `make seed-reset`.
...
1. Откройте `/learning` и выберите урок.
2. Прочитайте короткий теоретический блок (ориентир — 5 минут).
3. Нажмите **Создать тест и открыть IDE** — starter появится по рекомендуемому
   пути в `student_tests/` и сразу откроется в редакторе.
4. Реализуйте критерии и нажмите **Запустить**.
5. Платформа покажет baseline, статическую политику и обнаруженные мутации,
   сохранит попытку и обновит персональный прогресс.
6. После локальной проверки отправьте файл в Git; Jenkins и Allure остаются
   финальным независимым контуром.

Роль `MANAGER` получает страницу `/learning/manage` с прогрессом учеников.
...
## Встроенная IDE в интерфейсе приложения

После входа в приложении появляется пункт меню **IDE** (страница `/ide`,
доступна в development/test окружениях). Три панели:

- **Файлы решений** — всё содержимое `student_tests/**/test_*.py`;
- **редактор кода** (CodeMirror, подсветка Python) с сохранением по
  `Ctrl+S` и кнопкой **▶ Запустить**: тест выполняется на сервере платформы,
  вывод pytest показывается сразу под редактором;
- **поиск локаторов** — вводите путь страницы (`/login`, `/practice`,
  `/courses`…), получаете все её `data-testid` с тегом и текстом; кнопка
  «копировать» кладёт в буфер готовый селектор
  `page.get_by_test_id("...")`, рядом живой предпросмотр страницы.

Типовой цикл: открыть `/ide` → найти локаторы нужной страницы → написать тест
по образцу → ▶ Запустить → исправить → сохранить → `git push` в Jenkins/Allure.
Для полноценной разработки (дебаггер, UI-тесты Playwright) используйте также
code-server на http://localhost:8443 — оба инструмента работают с одними и
теми же файлами.
...
4. Jenkins уже запускается с локалью `ru` и часовым поясом `Europe/Moscow`.
   Переводы зависят от конкретного плагина: если у плагина нет русского
   каталога, его отдельные подписи останутся на английском.
5. Установите рекомендуемые плагины. Плагин **Allure Jenkins Plugin**
...
6. Создайте джобу: **New Item → Pipeline** и выберите
...
7. Настройте webhook Git-провайдера на Jenkins (polling `H/5 * * * *` остаётся
...
├── AUTOMATION_PRACTICE_CATALOG.md # каталог практических задач и критериев
...
│   ├── migrations/          # версионируемая схема PostgreSQL/SQLite
│   ├── requirements.txt
│   └── app/
│       ├── core/              # config, database, security (JWT/хеши), rate_limit
│       ├── learning/          # микро-уроки, starter, runner и критериальная оценка
